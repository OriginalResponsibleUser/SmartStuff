 ...
 */
metadata {
	definition (name: "Generic Camera", namespace: "jacobbraun", author: "patrick@patrickstuart.com") {
...
/*
    ...
    */
    tiles {
        standardTile("take", "device.image", width: 1, height: 1, canChangeIcon: true, canChangeBackground: true) {
            state "take", label: "Take", action: "Image Capture.take", icon: "st.camera.camera", backgroundColor: "#FFFFFF", nextState:"taking"
            state "taking", label:'Taking', action: "", icon: "st.camera.take-photo", backgroundColor: "#53a7c0"
            state "image", label: "Take", action: "Image Capture.take", icon: "st.camera.camera", backgroundColor: "#FFFFFF", nextState:"taking"
        }

        standardTile("refresh", "device.alarmStatus", decoration: "flat") {
...
def paramsGet = "http://10.13.37.20/img/snapshot.cgi"
...
def take() {
   ...
   log.trace "erroe!!! $e"
    ...
    log.trace "Requires Auth: $CameraAuth"
    log.trace "Uses which method: $CameraPostGet"
    ...
    def method = "GET"
    try {
    	if (CameraPostGet.toUpperCase() == "POST") h{
        	method = "POST"
        	}
        }
        ...
        log.trace e
        log.trace "You must not of set the perference for the CameraPOSTGET option"
    }
    
    log.trace "The method is $method"
    ...
    hubAction
    	...
    	log.trace "Hit Exception $e on $hubAction"
    }
